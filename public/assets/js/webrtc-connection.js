let localStream;
    ...
    var role = getParameterByName('role') === 'viewer'?1:2;
    var muted = getParameterByName('muted')?getParameterByName('muted'):'false';
    var subscribeForward = true;//getParameterByName('forward') === 'true'?true:false;
    var conf = {rtcConfiguration:{iceServers: [{urls:["stun:turn.goto-rtc.com:5060"],username:"citrixturnuser",credential:"turnpassword"},{urls:["turn:turn.goto-rtc.com:5060?transport=udp","turn:turn.goto-rtc.com:443?transport=tcp","turn:turn.goto-rtc.com:5060?transport=tcp"],username:"citrixturnuser",credential:"turnpassword"}]}};
    conference = new Ics.Conference.ConferenceClient();
    function renderVideo(stream){
        conference.subscribe(stream)
        .then((subscriptions)=>{
           let $video = $('<video id="'+stream.id+'" controls autoplay width="320" height="240">this browser does not supported video tag</video>');
           $video.get(0).srcObject = stream.mediaStream;
           $('.video-container').append($video);
           subscriptions.addEventListener('error', (err) => {
               $('#'+stream.id).remove();
           })
    ...
    window.onload = function() {
        ...
        if(role == 1){
            ...
            subscribeForward = false;
        }else{
            subscribeForward = true;
            $('.pushbtn').hide();
        }
        if(muted == 'true'){
            $('.pushbtn').remove();
        ...
        myRoom = '5bc5e923d5de477e5a13c699';//getParameterByName('room');
    ...
    function pushTalkEnable(){
           $('.pushbtn').html('Talking Enabled !!');
           $('.pushbtn').addClass('green');
           mixStream(myRoom, publicationGlobal.id, 'common')
    }
    function pushTalkDesable(){
        $('.pushbtn').html('Push To Talk');
        $('.pushbtn').removeClass('green');
        unmixStream(myRoom, publicationGlobal.id, 'common')
    }
                ...
                //$('.local video').get(0).srcObject = stream;
                    ...
                    conference.publish(localStream).then(publication => {
                        ...
                        if(role == 2){
                            ...
                            unmixStream(myRoom, publication.id, 'common')
                        }
